#!/usr/bin/env node
/**
 * autopg wrapper — finds and spawns the bun runtime from node_modules.
 *
 * v3.0.0 rename: this file was `bin/pgserve-wrapper.cjs` through v2.6.x.
 * The `autopg` bin is now the only CLI entry point; the legacy `pgserve`
 * bin is gone (clean cutover per pgserve-singleton-no-proxy Decision #1
 * + Felipe directive 2026-05-10). All previous `pgserve <verb>`
 * invocations now read `autopg <verb>`.
 *
 * Dispatch for every `autopg <verb>` (install / status / doctor /
 * update / trust / verify / gc / provision / create-app / config /
 * restart / ui / port / url / uninstall) lives downstream in
 * src/cli-install.cjs.
 *
 * Windows EBUSY fix: synchronous waiting + taskkill for reliable
 * process termination and file-handle cleanup.
 */
...
const __installSubcommands = new Set([
  ...
  // verbs. v3.0.0 dropped the `pgserve` bin; this wrapper is now the
  // single dispatcher for `autopg create-app` (was previously routed
  // through the same code via the now-removed pgserve bin alias).
  'create-app',
]);
