...
The repo will eventually move from `namastexlabs/pgserve` to `automagik-dev/autopg` (the last cutover step — separate operator action).
...
| Item | Reason |
|---|---|
| Repo rename `namastexlabs/pgserve` → `automagik-dev/autopg` | Felipe Q2 (2026-05-11): *"this will be the last part. we will change org too"*. Trust regex + `install.sh REPO=` + every `github.com/namastexlabs/pgserve` URL in the repo stays as-is until the actual `gh repo transfer` happens. Then a follow-up PR rewrites all references in lockstep. |
...
| 3 | Repo rename happens LAST | Felipe directive 2026-05-11, Q2: *"this will be the last part. we will change org too"* | Decouples the rename PR from the GitHub admin step. Trust regex + URLs left intact in v3.0.0 release; a follow-up cleanup PR rewrites them once `gh repo transfer namastexlabs/pgserve automagik-dev` runs. |
...
- [ ] (LATER) repo rename: `namastexlabs/pgserve` → `automagik-dev/autopg` + follow-up PR rewrites trust regex + install.sh REPO + GitHub URLs
...
| (follow-up) | Repo rename PR — only after `gh repo transfer namastexlabs/pgserve automagik-dev` | future |
...
- **GitHub redirect window** — after the eventual repo rename, GitHub auto-redirects but cosign cert subjects change from `namastexlabs/pgserve` → `automagik-dev/autopg`. The trust regex MUST update in lockstep with the repo rename PR; cannot lag.
