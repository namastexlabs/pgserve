...
> wish: ships the merged Wave 2 (G3 `autopg create-app` + manifest LOCK 1),
...
- **`autopg create-app <slug>`** — per-consumer app registration with manifest
  ...
  create time so subsequent `autopg verify --slug <slug>` invocations consult
...
- **Audit log rotation** — every `autopg gc` invocation now rotates audit
...
| Surface | Description |
|---------|-------------|
| `autopg create-app <slug>` | Per-consumer manifest LOCK 1 — writes `~/.autopg/<slug>/{admin,manifest}.json`, registers in `autopg_meta`, freezes trust roots |
| `autopg verify --slug <slug> <bin>` | Verify binary against locked roots from autopg_meta |
...
- `autopg gc` runs audit-log rotation at the start of every invocation
...
verification (no signing material to feed it). `autopg verify
...
~/.autopg/<slug>/` and re-run `autopg create-app <slug>`. Auto-regeneration
...
- **Wave A** — pgserve release-pipeline rebuild (signing artifacts);
  unblocks `autopg verify <pgserve-binary>` end-to-end.
