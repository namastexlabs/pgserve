#!/usr/bin/env bash
# ============================================================================
# pgserve — DEPRECATED LEGACY INSTALLER
# (renamed from install.sh to install-pgserve-legacy.sh in v2.6 — see
#  `.genie/wishes/autopg-distribution-cutover-finalize/WISH.md` G1)
#
# This is the original npm + pm2 installer. The cohort-canonical
# replacement is `install-autopg.sh`, which fetches a signed binary
# from GitHub Releases and verifies via `gh attestation verify`.
#
# Use this script only if you specifically need the npm-based install
# path (e.g. CI environments that already have node + bun and don't
# want to fetch the binary).
#
# Migration: `curl -fsSL https://raw.githubusercontent.com/namastexlabs/pgserve/main/install-autopg.sh | bash`
#
# Original docstring follows. Behavior is unchanged.
#
...
#
# Usage (legacy):
#   curl -fsSL https://raw.githubusercontent.com/namastexlabs/pgserve/main/install-pgserve-legacy.sh | bash
#
# With pinned version:
#   PGSERVE_VERSION=^2.1.1 curl -fsSL .../install-pgserve-legacy.sh | bash
#
# Local checkout:
#   bash install-pgserve-legacy.sh
#
# Idempotent — re-running is a no-op success when pgserve is already
# registered under pm2 with a healthy entry.
# ============================================================================
echo "[pgserve] install-pgserve-legacy.sh is deprecated; use install-autopg.sh going forward." >&2
